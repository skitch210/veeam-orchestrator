...
Write-Host "Restore Session ID: " $ec2Restore.Id

# Wait loop until restore is complete
do {
    $restored = aws ec2 describe-instances --filters "Name=tag-value,Values=$VMName" | ConvertFrom-Json
    Start-Sleep -Seconds 15
} until ($restored.Reservations -ne $null)
